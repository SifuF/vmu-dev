...
void VmuGraphics::createInstance() {
    ...
    std::vector<VkExtensionProperties> availablExtensions(availableExtensionCount);
    vkEnumerateInstanceExtensionProperties(nullptr, &availableExtensionCount, availablExtensions.data());
    std::cout << "available extensions:\n";
    for (const auto& extension : availablExtensions) {
...
void VmuGraphics::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
    createInfo = {};
...
void VmuGraphics::setupDebugMessenger() {
...
VkResult VmuGraphics::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDebugUtilsMessengerEXT* pDebugMessenger) {
...
void VmuGraphics::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator) {
...
void VmuGraphics::createSurface() {
    if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
        throw std::runtime_error("failed to create window surface!");
    }
}

void VmuGraphics::pickPhysicalDevice() {
...
void VmuGraphics::createLogicalDevice() {
...
void VmuGraphics::createSwapChain() {
...
void VmuGraphics::recreateSwapChain() {
...
void VmuGraphics::cleanupSwapChain() {
...
void VmuGraphics::createImageViews() {
...
void VmuGraphics::createRenderPass() {
...
void VmuGraphics::createGraphicsDescriptorSetLayout() {
...
void VmuGraphics::createGraphicsPipeline() {
...
void VmuGraphics::createFramebuffers() {
...
uint32_t VmuGraphics::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void VmuGraphics::createGraphicsCommandPoolAndBuffer() {
...
void VmuGraphics::createVertexBuffer() {
...
void VmuGraphics::createIndexBuffer() {
...
void VmuGraphics::createBuffer(VkDeviceSize imageSize, VkBufferUsageFlagBits bufferFlags, VkMemoryPropertyFlagBits memoryFlags, VkBuffer & buffer, VkDeviceMemory & memory) {
    VkBufferCreateInfo bufferInfo{};
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    ...
    uint32_t memoryTypeIndex = findMemoryType(typeFilter, memoryFlags);
...
void VmuGraphics::createUniformBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, static_cast<VkMemoryPropertyFlagBits>(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT), uniformBuffers[i], uniformBuffersMemory[i]);

        vkMapMemory(device, uniformBuffersMemory[i], 0, bufferSize, 0, &uniformBuffersMapped[i]);
    }
}

void VmuGraphics::createGraphicsDescriptorPool() {
...
void VmuGraphics::createGraphicsDescriptorSets() {
...
void VmuGraphics::createTextureImage() {
    ...
    imageInfo.extent.width = static_cast<uint32_t>(vmuWidth);
    imageInfo.extent.height = static_cast<uint32_t>(vmuHeight);
...
void VmuGraphics::createTextureImageView() {
...
void VmuGraphics::createTextureSampler() {
...
VkCommandBuffer VmuGraphics::beginSingleTimeCommands() {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    allocInfo.commandPool = graphicsCommandPool;
...
void VmuGraphics::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void VmuGraphics::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
...
void VmuGraphics::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VmuGraphics::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void VmuGraphics::createSyncObjects() {
...
void VmuGraphics::updateUniformBuffer(uint32_t currentImage) {
...
void VmuGraphics::allocateBufferMemoryAndBind(VkBuffer& buffer, VkDeviceMemory& memory, VkMemoryPropertyFlagBits flags) {
    VkMemoryRequirements bufferMemoryRequirements;
    vkGetBufferMemoryRequirements(device, buffer, &bufferMemoryRequirements);
    VkDeviceSize requiredMemorySize = bufferMemoryRequirements.size;
    uint32_t typeFilter = bufferMemoryRequirements.memoryTypeBits;
...
void VmuGraphics::copyBuffer(VkBuffer& srcBuffer, VkBuffer& dstBuffer, VkDeviceSize size, VkCommandBuffer& commandBuffer) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void VmuGraphics::copyBuffer(VkBuffer& srcBuffer, VkBuffer& dstBuffer, VkDeviceSize size) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
VkShaderModule VmuGraphics::createShaderModule(const std::vector<char>& code) {
...
VkSurfaceFormatKHR VmuGraphics::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR VmuGraphics::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D VmuGraphics::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
SwapChainSupportDetails VmuGraphics::querySwapChainSupport(VkPhysicalDevice device) {
...
bool VmuGraphics::isDeviceSuitable(VkPhysicalDevice device) {
...
bool VmuGraphics::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
QueueFamilyIndices VmuGraphics::findQueueFamilies(VkPhysicalDevice device) {
...
std::vector<const char*> VmuGraphics::getRequiredExtensions() {
...
bool VmuGraphics::checkValidationLayerSupport() {
    uint32_t layerCount;
...
std::vector<char> VmuGraphics::readFile(const std::string& filename) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL VmuGraphics::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
